---
{"publish":true,"created":"2025-07-03T10:08:40.740+02:00","modified":"2025-07-03T16:20:19.000+02:00","published":"2025-07-03T16:20:19.000+02:00","cssclasses":""}
---
...
- now create a file called `wg0.conf` (`wg0` is the usual name for a WireGuard interface) in `/etc/wireguard` with the following content
  ...
  - `PrivateKey` needs to be the contents of the `privatekey` file created [[Linux/Setup a WireGuard VPN server and connect a FRITZ!Box#^879ce6\|before]]
...
- for the FRITZ!Box client we now need to write the config file that we can then import to create the connection to this server; it needs the following content ^d7ac20
  ...
  - the `PrivateKey` must be the private key of the FRITZ!Box than can again be found in the *[[Linux/Setup a WireGuard VPN server and connect a FRITZ!Box#^8d7261\|WireGuard Settings]]*
  - the `[Peer]` section now contains the information of the WireGuard server
    - `PublicKey` is the server's public key can be found in the `publickey` file from [[Linux/Setup a WireGuard VPN server and connect a FRITZ!Box#^879ce6\|above]] or in the output of the [[Linux/Setup a WireGuard VPN server and connect a FRITZ!Box#^5c712a\|wg command]]
...
- this file can now be imported using the *WireGuard Assistant* after selecting *Couple Networks or Create a Custom Connection* in the first step, then selecting *Yes* for *Was this connection created on the remote peer?*
  ...
  - then select the file we created [[Linux/Setup a WireGuard VPN server and connect a FRITZ!Box#^d7ac20\|before]], give the connection a name and click *Finish*
