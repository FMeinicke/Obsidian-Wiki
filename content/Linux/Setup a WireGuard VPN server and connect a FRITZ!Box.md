---
publish: true
created: 2025-07-04T08:15:58.403+02:00
modified: 2025-07-03T16:20:19.000+02:00
published: 2025-07-03T16:20:19.000+02:00
---
...
- now create a file called `wg0.conf` (`wg0` is the usual name for a WireGuard interface) in `/etc/wireguard` with the following content
  ...
  - `PrivateKey` needs to be the contents of the `privatekey` file created [[#^879ce6|before]]
  - in the `[Peer]` section we specify the one and only client (this example is for connecting a VPS where the WireGuard server runs to a FRITZ!Box at home for allowing access to a local home server as the VPS runs a reverse proxy)
    - the `PublicKey` must be the client's public key which can be found on the _VPN (WireGuard)_ page under _Internet > Freigaben_ after clicking the _Show WireGuard settings_ button
...
- then we can start the server (ignore the warnings) and check the status of the interface and the server using the following commands ^5c712a
...
- for the FRITZ!Box client we now need to write the config file that we can then import to create the connection to this server; it needs the following content ^d7ac20
  ...
  - the `PrivateKey` must be the private key of the FRITZ!Box than can again be found in the _[[#^8d7261|WireGuard Settings]]_
  - the `[Peer]` section now contains the information of the WireGuard server
    - `PublicKey` is the server's public key can be found in the `publickey` file from [[#^879ce6|above]] or in the output of the [[#^5c712a|wg command]]
...
- this file can now be imported using the _WireGuard Assistant_ after selecting _Couple Networks or Create a Custom Connection_ in the first step, then selecting _Yes_ for _Was this connection created on the remote peer?_
  ![[FRITZ!Box WireGuard Assistant 1.png]]
  ![[FRITZ!Box WireGuard Assistant 2.png]]
  - then select the file we created [[#^d7ac20|before]], give the connection a name and click _Finish_
  - after this you should see the connection under the _WireGuard Network Connections_ in the overview
